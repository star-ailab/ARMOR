...
## Load parameters
parameters = load_parameters("../src/parameters_dro_symm_f_worig.ini")
...
alpha_ = float(parameters["hyperparam"]["alpha"])
...
## Not used.
def pgd_linf_OT_NoProjection(model, X, y, lambda_, epsilon=0.1, step_alpha=0.1, num_iter=40, randomize=False):
    if randomize:
        delta = torch.rand_like(X, requires_grad=True)
        delta.data = delta.data * 2 * epsilon - epsilon
    else:
...
def pgd_linf_OT_NoProjection2(model, x, y, lambda_, epsilon=0.3, step_alpha=0.01, num_iter=40):
    # some book-keeping
...
## Our Outer losses
def f_star(y, alpha_):
    return (1.0/alpha_) * ((alpha_-1)**(alpha_/(alpha_-1))) * ( (torch.relu(y))**(alpha_/(alpha_-1)) ) + 1.0/(alpha_*(alpha_-1))
...
class min_loss_fdiv_symmetric (nn.Module):
    ...
    def forward(self, x_adv_model, y, x_adv, x):
        ...
        minLoss = (eps * (self.lambda_)) + self.nu +\
                  (self.lambda_) * ( torch.mean(f_star(maxLoss - ((1.0/self.lambda_)*self.nu), alpha_))  ) 
...
model_cnn_robust_dro_f_symm_worig = nn.Sequential(nn.Conv2d(1, 32, 3, padding=1), nn.ReLU(),
...
opt = optim.SGD(model_cnn_robust_dro_f_symm_worig.parameters(), lr=1e-1)
...
print ( "criterion: ", type(criterion), "dro_epsilon: ", float(parameters["hyperparam"]["eps"]), "L: ", float(parameters["hyperparam"]["ot_norm_weight"]),\
                        ...
                        "lr_lambda_nu: ",  float(parameters["hyperparam"]["loss_learning_rate"]), \
                        )

print("train_err,       adv_acc,       adv_FPR,        adv_FNR,       test_acc,       test_FPR,       test_FNR")

for t in range(10):
    train_err, train_loss = epoch_adversarial_dro(train_loader, model_cnn_robust_dro_f_symm_worig, pgd_linf_OT_NoProjection2, criterion.lambda_, opt, optimizer_loss_params)
    test_err, test_loss, test_FPR, test_FNR = epoch(test_loader, model_cnn_robust_dro_f_symm_worig)
    adv_err, adv_loss, adv_FPR, adv_FNR = epoch_adversarial_dro_eval(test_loader, model_cnn_robust_dro_f_symm_worig, pgd_linf_OT_NoProjection2, criterion.lambda_)
    if t == 4:
...
torch.save(model_cnn_robust_dro_f_symm_worig, "model_cnn_robust_dro_f_symm_worig_ours_wo_noise.pt")


##################### Evaluation

fgsm_err , _, fgsm_FPR, fgsm_FNR = epoch_adversarial(test_loader, model_cnn_robust_dro_f_symm_worig, fgsm)

print("fgsm acc", 1.0-fgsm_err, "fgsm FPR: ", 100.0*fgsm_FPR, "fgsm FNR: ", 100.0*fgsm_FNR)

pgd_err , _, pgd_FPR, pgd_FNR = epoch_adversarial(test_loader, model_cnn_robust_dro_f_symm_worig, pgd_linf, num_iter=40)
