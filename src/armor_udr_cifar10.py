...
def attack_udr(model, X, y, epsilon, alpha, attack_iters, restarts,
    ...
    for _ in range(restarts):
        ...
        for _ in range(attack_iters):
            ...
            tau = alpha
            loss = F.cross_entropy(output, y)
            loss_c = lamda.detach() / tau * 0.5*((delta-epsilon*torch.sign(delta))**2*(torch.abs(delta)>=epsilon)).sum()
...
def get_args():
    ...
    parser.add_argument('--model', default='ResNet')##PreActResNet18
    ...
    parser.add_argument('--nu', default=-0.35, type=float, help='our nu')
    parser.add_argument('--r', default=0.5, type=float, help='our r')
    
    parser.add_argument('--loss_learning_rate', default=0.000004, help='armor lambda and nu learning rate')
    parser.add_argument('--alph', default=6.0, help='armor alpha')
    parser.add_argument('--scale', default=0.1, help='scale cost')
...
def main():
    ...
    model_name = 'ds=cifar10_model={}_method={}'.format(args.model, '_armor_udr_noawp_ours')
    ...
    class min_loss_fdiv_symmetric (nn.Module):
        def __init__(self):
            ...
            self.lambda_ = nn.Parameter(torch.tensor(args.lamda_init, requires_grad=True))
        ...
        def forward(self, model, x_natural, x_adv, y, tau):
            ...
            maxLoss = F.cross_entropy(model(normalize(x_adv)), y, reduction='none') - (args.scale) * self.lambda_ / tau * 0.5*((delta-epsilon*torch.sign(delta))**2*(torch.abs(delta)>=epsilon)).sum(dim=(1,2,3))
            minLoss = (args.r * (self.lambda_)) + self.nu +\
                      (self.lambda_) * ( torch.mean(f_star((1.0/self.lambda_)* maxLoss - ((1.0/self.lambda_)*self.nu), args.alph))  ) 
            if(torch.rand(1)>0.990):
    ...
    optimizer_loss_params = torch.optim.AdamW(list(criterion.parameters()), lr=args.loss_learning_rate)
