...
parser.add_argument('--beta', default=19.0,
...
parser.add_argument('--nu', default=-0.35, help='armor initial nu')
...
class min_loss_fdiv_symmetric (nn.Module):
    ...
    def forward(self, model, x_natural, y, optimizer):
        ...
        maxLoss = (1.0/self.lambda_)*(trades_loss(model=model, x_natural=x_natural, y=y.to(device), optimizer=optimizer, device=device, beta=args.beta))
...
def train(args, model, device, train_loader, optimizer, optimizer_loss_params, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss_robust = criterion(model=model,
                           x_natural=data,
                           y=target,
                           optimizer=optimizer)
