...
## Load parameters
parameters = load_parameters("../src/parameters_dro_symm_f.ini")
...
def epoch_adversarial_dro(loader, model, attack, lambda_, opt=None, optimizer_loss_params=None, **kwargs):
    ...
    for X,y in loader:
        ...
        if opt:
            ...
            opt.step()
            optimizer_loss_params.step()
            ## failsafe for lambda in case it is negative. Lambda should not be negative unless the learning rate is too large.
            if (lambda_<0):
...
model_cnn_robust_dro_f_symm = nn.Sequential(nn.Conv2d(1, 32, 3, padding=1), nn.ReLU(),
...
opt = optim.SGD(model_cnn_robust_dro_f_symm.parameters(), lr=1e-1)
...
print ( "criterion: ", type(criterion), "dro_epsilon: ", float(parameters["hyperparam"]["eps"]), "L: ", float(parameters["hyperparam"]["ot_norm_weight"]),\
                        ...
                        "beta: ", float(parameters["hyperparam"]["beta"]), "dro_alpha: ", float(parameters["hyperparam"]["alpha"]),\
...
for t in range(10):
    train_err, train_loss = epoch_adversarial_dro(train_loader, model_cnn_robust_dro_f_symm, pgd_linf_OT_NoProjection2, criterion.lambda_, opt, optimizer_loss_params)
    test_err, test_loss, test_FPR, test_FNR = epoch(test_loader, model_cnn_robust_dro_f_symm)
    adv_err, adv_loss, adv_FPR, adv_FNR = epoch_adversarial_dro_eval(test_loader, model_cnn_robust_dro_f_symm, pgd_linf_OT_NoProjection2, criterion.lambda_)
    if t == 4:
...
torch.save(model_cnn_robust_dro_f_symm.state_dict(), "model_cnn_robust_dro_f_symm.pt")


##################### Evaluation

fgsm_err , _, fgsm_FPR, fgsm_FNR = epoch_adversarial(test_loader, model_cnn_robust_dro_f_symm, fgsm)

print("fgsm acc", 1.0-fgsm_err, "fgsm FPR: ", 100.0*fgsm_FPR, "fgsm FNR: ", 100.0*fgsm_FNR)

pgd_err , _, pgd_FPR, pgd_FNR = epoch_adversarial(test_loader, model_cnn_robust_dro_f_symm, pgd_linf, num_iter=40)
