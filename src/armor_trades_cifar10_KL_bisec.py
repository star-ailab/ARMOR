...
parser.add_argument('--pgd_interval', default=20, help='PGD attack interval')
...
parser.add_argument('--k_', default=0.1, help='lower-boud ARMOR loss via lregularization')
...
def eval_adv_test_whitebox(model, device, test_loader, epoch, attacks):
    ...
    pgd_robust_acc = 0.
    if ('pgd' in attacks and epoch % args.pgd_interval == 0. and epoch>=80) or ('pgd' in attacks and epoch in [90,95,100,105,110,120,125,130,135]):
        torch.manual_seed(args.seed)
    ...
    ## Autoattack:
    if ('aa' in attacks and epoch % args.autoattack_interval == 0) or ('aa' in attacks and pgd_robust_acc>=0.535): ##('aa' in attacks and epoch % args.autoattack_interval == 0 and pgd_robust_acc>=0.53) or :
        torch.manual_seed(args.seed)
