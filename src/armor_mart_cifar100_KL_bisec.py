...
parser.add_argument('--epsilon', default=0.01, help='perturbation size')
parser.add_argument('--num-steps', default=10, help='perturbation number of steps')
parser.add_argument('--step-size', default=0.001, help='perturb step size')
...
parser.add_argument('--pgd_interval', default=10, help='PGD attack interval')
parser.add_argument('--autoattack_interval', default=10, help='autoattack attack interval')
parser.add_argument('--save-freq', '-s', default=150, type=int, metavar='N', help='save frequency')


## armor params
parser.add_argument('--eps', default=0.002, help='armor epsilon')
parser.add_argument('--lambd', default=1.0, help='armor initial lambda')
parser.add_argument('--t', default=0.8, help='natural vs. adv. loss (between 0 and 1)')
...
trainset = torchvision.datasets.CIFAR100(root='../data_attack/', train=True, download=True, transform=transform_train)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=0)
testset = torchvision.datasets.CIFAR100(root='../data_attack/', train=False, download=True, transform=transform_test)
...
class armor (nn.Module):
    ...
    def forward(self, model, x_natural, y, optimizer):
        ...
        losses = mart_loss(model=model, x_natural=x_natural, y=y.to(device), optimizer=optimizer, device=device, beta=args.beta)
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss_nat = loss_fct(model(data), target).mean()
...
def eval_adv_test_whitebox(model, device, test_loader, epoch, attacks):
    ...
    pgd_robust_acc = 0.
    if ('pgd' in attacks and epoch % args.pgd_interval == 0. and epoch>=90) or ('pgd' in attacks and epoch in [115,125,135]):
        torch.manual_seed(args.seed)
        ...
        print('pgd_robust_acc: ', pgd_robust_acc )
    
    ## Autoattack:
    if ('aa' in attacks and epoch % args.autoattack_interval == 0 and epoch>=90 and pgd_robust_acc>=0.4955) or ('aa' in attacks and pgd_robust_acc>=0.499): ##('aa' in attacks and epoch % args.autoattack_interval == 0 and pgd_robust_acc>=0.53)
        torch.manual_seed(args.seed)
        for X_,y_ in test_loader_1000:
            X_,y_ = X_.to(device), y_.to(device)
            break
        adversary = AutoAttack(model, norm='Linf', eps=args.epsilon, verbose= False, device="cuda")
