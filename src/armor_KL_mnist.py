...
from utils.utils import clip_tensor
...
## Load parameters
parameters = load_parameters("../src/parameters_dro_symm_KL.ini")
...
def pgd_linf_OT(model, X, y, lambda_, epsilon=0.1, step_alpha=0.1, num_iter=20, randomize=False):
    if randomize:
        delta = torch.rand_like(X, requires_grad=True)
        delta.data = delta.data * 2 * epsilon - epsilon
    else:
    ...
    for t in range(num_iter):
        ...
        loss.mean().backward()
        delta.data = (delta + step_alpha*delta.grad.detach().sign()).clamp(-epsilon,epsilon)
        delta.grad.zero_()
    return delta.detach()
...
model_cnn_robust_dro_KL_symm = nn.Sequential(nn.Conv2d(1, 32, 3, padding=1), nn.ReLU(),
...
opt = optim.SGD(model_cnn_robust_dro_KL_symm.parameters(), lr=1e-1)
...
for t in range(10):
    train_err, train_loss = epoch_adversarial_dro(train_loader, model_cnn_robust_dro_KL_symm, pgd_linf_OT_NoProjection2, criterion.lambda_, opt, optimizer_loss_params)
    test_err, test_loss, test_FPR, test_FNR = epoch(test_loader, model_cnn_robust_dro_KL_symm)
    adv_err, adv_loss, adv_FPR, adv_FNR = epoch_adversarial_dro_eval(test_loader, model_cnn_robust_dro_KL_symm, pgd_linf_OT_NoProjection2, criterion.lambda_)
    if t == 4:
...
torch.save(model_cnn_robust_dro_KL_symm.state_dict(), "model_cnn_robust_dro_KL_symm.pt")


##################### Evaluation

## print("pgd_linf_OT, 40 iter: ", epoch_adversarial_dro_eval(test_loader, model_cnn_robust_dro_KL_symm, pgd_linf_OT_NoProjection2, criterion.lambda_, num_iter=40))


fgsm_err , _, fgsm_FPR, fgsm_FNR = epoch_adversarial(test_loader, model_cnn_robust_dro_KL_symm, fgsm)

print("fgsm acc", 1.0-fgsm_err, "fgsm FPR: ", 100.0*fgsm_FPR, "fgsm FNR: ", 100.0*fgsm_FNR)

pgd_err , _, pgd_FPR, pgd_FNR = epoch_adversarial(test_loader, model_cnn_robust_dro_KL_symm, pgd_linf, num_iter=40)
